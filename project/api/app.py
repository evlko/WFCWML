...
from project.wfc.advisor import Advisor, GreedyAdvisor, RandomAdvisor
from project.wfc.factory import ApiFactory, JsonFactory
from project.wfc.grid import Grid, Rect
from project.wfc.judge import AlwaysContinueJudge, Judge, RandomJudge
...
class WFCConfig(BaseModel):
    ...
    images_folder: str = Field(
        description="Path to images folder (e.g., 'sprites/forest/')"
    )
    patterns: list[dict[str, Any]] = Field(
        description="List of pattern definitions with rules"
    )
...
class GenerateRequest(BaseModel):
    ...
    generations: int = Field(
        default=1, ge=1, description="Target number of successful generations"
    )
    config: WFCConfig | None = Field(
        default=None,
        description="Optional WFC configuration. If not provided, uses default config from project settings.",
    )
    judge_id: int = Field(
        default=0, description="Judge ID: 0=AlwaysContinue, 1=90%Continue"
    )
...
async def generate(request: GenerateRequest):
    ...
    if request.config:
        ...
        factory = ApiFactory(
            images_folder=request.config.images_folder, patterns_data=processed_patterns
        )
    else:
    ...
    return {"grid": grid.uids}


def main():
    """Run the FastAPI application using uvicorn."""
    uvicorn.run("project.api.app:app", host="::", port=8000, reload=True)
